...
=======
                      warning = FALSE,
>>>>>>> 391225188a14fe4fcc1da9356acce2de5c8359c9
                      fig.align = "center")
```
...
-    optimal number of clusters 참고 or 주관적으로 결정함 
-    optimal number of clusters도 nstart, nboot 설정에 따라 유동적임 
...
# nboot	: integer, number of Monte Carlo ("bootstrap") samples. Used only for determining the number of clusters using gap statistic. recommend value = 500


set.seed(123)
fviz_nbclust(df_6, kmeans, method = "gap_stat", nstart = 25, nboot = 50)

fviz_nbclust(df_6, kmeans, method = "wss") +
    geom_vline(xintercept = 4, linetype = 2)+
  labs(subtitle = "Elbow method")

set.seed(123)
fviz_nbclust(df_6, kmeans, method = "silhouette")
...
-    그냥 prcomp 써도 상관없음
-    FactoMineR 패키지의 PCA는 scaling이 default임 
-    graph = T로 설정했을 때 시각화까지 수행해줌 
...
-   예를 들어 Pole.vault의 경우 PC1의 가중치는 -0.21, PC2의 가중치는 0.8이므로 해당 좌표(-0.21, 0.8)에 점이 찍힘 
...
```{r}
set.seed(123)
res <- hcut(USArrests, k = 3, stand = T)
res
```
...
df_k <- ads.s %>% 
  select(interest, Impressions, Clicks, Spent, Total_Conversion) 
...
```{r}
set.seed(123)
km.res <- kmeans(df_k, 4, nstart = 25)
km.res$cluster %>% head()

clust = data.frame(clust = km.res$cluster)
...
-    bind_cols는 error가 날 수도 있으므로 cbind 사용 추천

```{r}
df_k <- cbind(df_k, clust)

newdat <- ads.s %>% 
  select(-c(interest, Impressions, Clicks, Spent, Total_Conversion)) %>% 
  cbind(df_k)
...
newdat$xyz_campaign_id <- factor(newdat$xyz_campaign_id, labels = c("A", "B", "C"))

splits <- initial_split(newdat, prop = 0.7, strata = xyz_campaign_id)
...
rf_gridsearch <- train(xyz_campaign_id ~ .,             
                       data = train,               
                       method = 'rf',  
                       trControl = control,
                       tuneGrid = tunegrid,
...
cmatrix <- caret::confusionMatrix(pred, test$xyz_campaign_id) 
cmatrix
...
# PCA and modeling tutorial {.tabset .tabset-fade}
...
-   step_pca()
    ...
    -   위의 예시를 보면 주성분별 Cumulative Proportion이 PC4 : 0.79, PC5 : 0.84인데 treshold를 0.80으로 잡으면 PC5까지 선택됨(미만이면 초과해서 주성분이 선택됨. 주의 필요)
