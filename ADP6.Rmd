...
-    card_category, education_level, marital_status 변수 범주 불균형 처리
...
plot(varImp(logit2_gridsearch, scale = F))
...
-   predict type = 'raw' or 'prob' 만 구분해서 제출만 하면 됨 
...
-   모수가 적은 모형이 합리적인 모형이므로(추가해도 무방함)
...
```{r}
confint(Arima(flow_sdif, order=c(2,0,0), seasonal=list(order=c(0,1,1),period=12)))
confint(Arima(flow_sdif, order=c(1,0,1), seasonal=list(order=c(0,1,1),period=12)))
...
-   모형 가정 만족
-   과대적합 결과 추가된 모수 비유의
-   **ARIMA(1, 0, 0)(0, 1, 2)\_12** 잠정 모형으로 선택
...
-   aic, bic 비교 결과 **ARIMA(1, 0, 0)(0, 1, 2)\_12**를 최종 모형으로 선택
